...
fn test__exists() {
    ...
    assert(
        WorldInteractionsImpl::get_owner_of(@state, token_id).address == ZERO(), 'Invalid owner'
    );
    ...
    assert(
        WorldInteractionsImpl::get_owner_of(@state, token_id).address == RECIPIENT(),
        'Invalid owner'
    );
...
fn test__mint() {
    ...
    assert_state_before_mint(@state, recipient);
    InternalImpl::_mint(ref state, recipient, TOKEN_ID);
    assert_event_transfer(ZERO(), recipient, token_id);

    assert_state_after_mint(@state, recipient, token_id);
}
...
fn test__burn() {
    ...
    assert(
        WorldInteractionsImpl::get_owner_of(@state, TOKEN_ID).address == ZERO(), 'Ownership after'
    );
    assert(ERC721Impl::balance_of(@state, OWNER()) == 0, 'Balance of owner after');
    assert(
        WorldInteractionsImpl::get_token_approval(@state, TOKEN_ID).address == ZERO(),
        'Approval after'
    );
}
...
fn assert_state_before_mint(state: @ERC721::ContractState, recipient: ContractAddress) {
    assert(ERC721Impl::balance_of(state, recipient) == 0, 'Balance of recipient before');
}
